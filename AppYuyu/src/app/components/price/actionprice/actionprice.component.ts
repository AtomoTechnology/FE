...
export class ActionpriceComponent implements OnInit {
  ...
  Update() {
    this.genericService
      .Put(this.actionForm.value, this.ctrl.price)
      .subscribe((data: any) => {
        if (data.status) {
          setTimeout(() => {
          ...
          this.messageService.Success(
            'Actualizar precio',
            'El precio fue actualizado con exito'
          );
        } else {
          this.messageService.Error('Error', 'El precio no se actualizó');
        }
      });
  }
  Save() {
    this.genericService
      .Post(this.actionForm.value, this.ctrl.price)
      .subscribe((data: any) => {
        if (data.status) {
          setTimeout(() => {
            this.router.navigate(['/prices']);
          ...
          this.messageService.Success(
            'Crear precio',
            'El precio fue creado con exito'
          );
        } else {
          this.messageService.Error('Error', 'El precio no fue creado ');
        }
      });
  }
}
